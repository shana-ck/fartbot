...
__export(src_exports3, {
  ...
  toSimplifiedISOSafe: () => toSimplifiedISOSafe,
  ui8ToArrayBuffer: () => ui8ToArrayBuffer,
  ui8ToBuffer: () => ui8ToBuffer,
  utf8Len: () => utf8Len,
...
function toSimplifiedISOSafe(dateStr) {
  ...
  if (!(0, import_iso_datestring_validator.isValidISODateString)(iso)) {
    return new Date(0).toISOString();
  }
  return iso;
}
...
var fileExists = async (location) => {
  try {
    await import_promises.default.access(location, import_fs.constants.F_OK);
    return true;
  } catch (err) {
...
var verifyCidForBytes = async (cid, bytes) => {
  const digest2 = await sha256.digest(bytes);
  const expected = CID.createV1(cid.code, digest2);
...
var sha256ToCid = (hash, codec) => {
  const digest2 = digest_exports.create(sha256.code, hash);
  return CID.createV1(codec, digest2);
};
var sha256RawToCid = (hash) => {
  return sha256ToCid(hash, code2);
};
var VerifyCidTransform = class extends import_stream.Transform {
  constructor(cid) {
    super();
    this.cid = cid;
    this.hasher = import_crypto2.default.createHash("sha256");
  }
...
function setExtractor(extractStrings) {
  ...
  function readString(headerLength) {
    return function readString2(length2) {
      ...
      position += length2;
      return string2.slice(0, length2);
    };
  }
}
...
var Encoder2 = class extends Decoder2 {
  constructor(options) {
    ...
    } : textEncoder3 && textEncoder3.encodeInto ? function(string2, position3) {
      return textEncoder3.encodeInto(string2, target.subarray(position3)).written;
    } : false;
    let encoder = this;
...
var cloneStream = (stream) => {
  const passthrough = new import_stream3.PassThrough();
  forwardStreamErrors(stream, passthrough);
  return stream.pipe(passthrough);
};
var streamSize = async (stream) => {
  let size = 0;
  for await (const chunk of stream) {
    size += chunk.length;
  }
...
var byteIterableToStream = (iter) => {
  return import_stream3.Readable.from(iter, { objectMode: false });
};
...
var MaxSizeChecker = class extends import_stream3.Transform {
