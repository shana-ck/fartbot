...
export class Post extends PostReference {
    ...
    async fetchThreadView() {
        const response = await this.bot.agent.app.bsky.feed.getPostThread({ uri: this.uri }).catch((e) => {
            throw new Error("Failed to fetch post like count", { cause: e });
        });
        if (!AppBskyFeedDefs.isThreadViewPost(response.data.thread)) {
            throw new Error(`Could not fetch post ${this.uri}. `
    ...
    static fromView(view, bot) {
        if ((!AppBskyFeedPost.isRecord(view.record)))
    ...
    static fromThreadView(view, bot) {
        ...
        const parent = AppBskyFeedDefs.isThreadViewPost(view.parent)
            ...
            : undefined;
        const children = view.replies?.map((reply) => AppBskyFeedDefs.isThreadViewPost(reply) ? Post.fromThreadView(reply, bot) : undefined)?.filter((reply) => reply !== undefined);
